...
def main(model_name: str, intention: str) -> None:
    ...
    for profile_key, profile_text in profile_data.items():
        print(profile_text)
        output_path = f"{OUTPUT_DIR}/{intention}/{model_name}_{profile_key}.json"
        os.makedirs(os.path.dirname(output_path), exist_ok=True)
        if os.path.exists(output_path):
            with open(output_path, "r", encoding="utf-8") as f:
                existing_results = json.load(f)
        else:
            existing_results = {"dialogue": {}}

        final_results = deepcopy(existing_results)
        completed_scene_ids = set(existing_results.get("dialogue", {}).keys())
        print(f"[{profile_key}] Already processed scene IDs: ", completed_scene_ids)
        ...
        for scene_id, scene_data in input_data.get("dialogue", {}).items():
            ...
            for name, role_data in scene_data.items():
                ...
                for turn_id in sorted(dialogue_results.keys(), key=lambda x: int(x)):
                    ...
                    raw_output = analyze_dialogue(model_name, name, role_data, history_dialogue, judge_sentences, profile_text)
...
if __name__ == "__main__":
    # "gemini-2-flash", "gemini-1.5-flash",  "gpt-3.5-turbo"
